...
const CASES = [
  ...
  },
  {
    ...
    page: { sdp: 'none', driver: `
      ...
      await t.wait(50);
      t.snapshot({ fb: t.has('#setupFallback'), consolation: t.has('#setupFallbackConsole'), urls: t.ops('iframe').map(function (o) { return o.url; }), txt: (document.querySelector('#setupFallback') || {}).textContent || '' });
    ` },
    check: r => [
      ['兜底面板可见', r.res.snap.fb === true],
      ['只引导换用 Edge / Chrome，不再引导改文件', /Edge/.test(r.res.snap.txt) && /Chrome/.test(r.res.snap.txt) && !/记事本|secrets\.json/.test(r.res.snap.txt), JSON.stringify(r.res.snap.txt.slice(0, 80))],
      ['面板内不再有可点入口（零协议调用）', r.res.snap.urls.length === 0, JSON.stringify(r.res.snap.urls)],
      ['控制台向导入口已移除', r.res.snap.consolation === false],
