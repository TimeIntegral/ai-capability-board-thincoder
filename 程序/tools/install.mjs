...
// 用法：node <程序>/tools/install.mjs   （或双击「快捷操作」里的「安装定时任务.bat」）
// 开关：--skip-verify  不跑收尾的「首次采集验证」（看板里的设置向导会自己试采一次，避免连着采两遍）
...
const QUICK = path.join(ROOT, '快捷操作');     // 用户双击的脚本
...
// 1) 计划任务
step('计划任务已注册', () => {
  const vbs = path.join(QUICK, '立即采集一次.vbs');
...
step('桌面快捷方式已创建', () => {
  const dash = path.join(QUICK, '启动看板.vbs');
  const ico = path.join(ROOT, 'icon.ico');
...
step('托盘图标已注册', () => {
  ...
  const vbs = path.join(QUICK, '托盘图标.vbs');
  if (!fs.existsSync(vbs)) throw new Error(`未找到 ${vbs}`);
...
Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
  Where-Object { $_.CommandLine -and $_.CommandLine.Contains('"${path.join(PROG, 'tools', 'tray.ps1')}"') } |
