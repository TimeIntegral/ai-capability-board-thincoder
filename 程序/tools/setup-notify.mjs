...
// 用法：node 程序/tools/setup-notify.mjs --ok        （安装完成）
//       node 程序/tools/setup-notify.mjs --fail      （有步骤没成功）
//       node 程序/tools/setup-notify.mjs --emit-b64  （打印三段载荷，供 启用自动采集.vbs 内嵌；零副作用、不弹框）
//
// 为什么有这个文件：.vbs 必须保持纯 ASCII（Windows 脚本宿主按 GBK 读，中文会吞换行），
// 而用户裁定「完成提示必须中文、必须有」——中文写在这里（UTF-8 正常），
// 弹框走 PowerShell -EncodedCommand（UTF-16LE base64，与 install.mjs 的 ps() 同法），中文零编码风险、零新依赖。
...
const TEXT = {
  ...
  noinstall: '没找到要用的安装脚本，这次没有开始安装。\n\n请把「启用自动采集.vbs」放回安装目录的「快捷操作」文件夹里，再双击一次。',
};
...
if (process.argv[1] && process.argv[1].endsWith('setup-notify.mjs')) {
  if (arg === '--emit-b64') {
    ...
    console.error('用法：node 程序/tools/setup-notify.mjs --ok | --fail | --emit-b64');
