...
// 本文件住在 <根>/程序/lib/：项目根（dashboard.html、config.json、data/ 那一层）在它上面两级。
// 这里是全仓「项目根」的唯一权威定义 —— 其余模块一律 import ROOT_DIR，不要再各自数目录层级。
...
//   ① config.dashboard.cards[p] === false → 不显示（连接窗口里那个「看板显示这一家」勾选框写这里）；
...
// / 「AI 能力构成」的模型画像 —— 画像出自 Codex 会话记录，于是跟着 Codex 的显示状态走）。
