// 规则引擎测试：node tools/test-rules.mjs [--slow]
...
import { readHistoryRows } from '../lib/common.mjs';
...
console.log('\n=== 测试 L：状态文件契约（tools/setup-check.mjs → data/setup-status.json） ===');
{
  ...
  try {
    ...
    out1 = execFileSync(process.execPath, [path.join(ROOT_DIR, 'tools', 'setup-check.mjs')], { encoding: 'utf8', timeout: 60000, stdio: ['ignore', 'pipe', 'pipe'] });
    raw1 = fs.readFileSync(statusPath, 'utf8');
    st = JSON.parse(raw1);
    fs.writeFileSync(configPath, '{broken');
    execFileSync(process.execPath, [path.join(ROOT_DIR, 'tools', 'setup-check.mjs')], { encoding: 'utf8', timeout: 60000, stdio: ['ignore', 'pipe', 'pipe'] });
...
console.log('\n=== 测试 N：中文提示载荷（两态 + 守卫态 + .vbs 内嵌载荷） ===');
{
  ...
  const vbsBuf = fs.readFileSync(path.join(ROOT_DIR, '启用自动采集.vbs'));
...
console.log('\n=== 测试 O：运行时文件不含密钥（真机扫描） ===');
if (!SLOW) {
  console.log('  （重 IO：起真实采集进程——默认跳过，单跑用 node tools/test-rules.mjs --slow）');
} else {
  const secretsPath = path.join(ROOT_DIR, 'secrets.json');
  const watch = [
    secretsPath,
  ...
  try {
    fs.writeFileSync(secretsPath, JSON.stringify({ deepseek: SENT }, null, 1));
    outs += execFileSync(process.execPath, [path.join(ROOT_DIR, 'tools', 'setup-check.mjs')], { encoding: 'utf8', timeout: 60000 });
    // 真实采集一次（含联网；失败属预期——本轮只验「不外泄」）
    try {
      outs += execFileSync(process.execPath, [path.join(ROOT_DIR, 'collect.mjs')], { encoding: 'utf8', timeout: 180000, stdio: ['ignore', 'pipe', 'pipe'] });
