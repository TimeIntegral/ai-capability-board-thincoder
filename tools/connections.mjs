...
function run(script, args = [], timeout = 120000) {
  execFileSync(process.execPath, [path.join(root, script), ...args], { cwd: root, timeout, windowsHide: true, stdio: 'ignore' });
}
async function main() {
  if (process.argv.includes('--save')) {
    ...
    execFileSync('powershell.exe', ['-NoProfile', '-STA', '-ExecutionPolicy', 'Bypass', '-File', path.join(root, 'tools/connections.ps1'), '-NodeExe', process.execPath], { env, windowsHide: true, stdio: 'ignore' });
  }
}
