...
test('native window passes secrets only through stdin and clears input', { skip: process.platform !== 'win32' }, t => {
  ...
  fs.copyFileSync(path.join(import.meta.dirname, 'connections.ps1'), path.join(dir, 'tools/connections.ps1'));
  fs.writeFileSync(path.join(dir, 'tools/connections.mjs'), `let text=''; for await (const part of process.stdin) text+=part; const p=JSON.parse(text); if(process.argv.length!==3 || process.argv[2]!=='--save' || p.keys.deepseek!=='fixture-value' || p.platforms.deepseek!==true) process.exit(1); process.stdout.write('连接测试通过');`);
  const env = { ...process.env }; delete env.PSModulePath;
  execFileSync('powershell.exe', ['-NoProfile', '-STA', '-ExecutionPolicy', 'Bypass', '-File', path.join(dir, 'tools/connections.ps1'), '-NodeExe', process.execPath, '-PreviewPath', path.join(dir, 'preview.png'), '-SmokeTest'], { env, timeout: 25000, windowsHide: true, stdio: 'pipe' });
