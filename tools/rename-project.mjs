...
//
//   cd /d <项目所在目录>
//   node 20260906-AI大模型额度看板\tools\rename-project.mjs 20260906-AI能力看板
//
...
const OLD_ROOT = path.dirname(import.meta.dirname);          // 当前项目根（改名前）

const log = m => console.log(m);
const die = m => { console.error('❌ ' + m); process.exit(1); };

if (!newName) die('用法：node tools/rename-project.mjs <新文件夹名> [--dry]');
...
if (cwdResolved === OLD_ROOT.toLowerCase() || cwdResolved.startsWith(OLD_ROOT.toLowerCase() + path.sep)) {
  die(`请在项目**外**执行（当前工作目录在项目内）：\n   cd /d "${parent}"\n   node "${path.relative(parent, path.join(OLD_ROOT, 'tools', 'rename-project.mjs'))}" ${newName}`);
}
...
try {
  const out = execFileSync('node', [path.join(NEW_ROOT, 'tools', 'install.mjs')], { encoding: 'utf8', timeout: 300000, cwd: NEW_ROOT });
  log('④ 重新注册完成：');
  for (const line of out.trim().split('\n')) log('   ' + line);
} catch (e) {
  log('④ 重新注册失败，请手动执行：node "' + path.join(NEW_ROOT, 'tools', 'install.mjs') + '"');
