...
export function prepareUpgrade(root = ROOT, { isolated = false } = {}) {
  ...
  const files = fs.existsSync(manifestPath) ? JSON.parse(fs.readFileSync(manifestPath, 'utf8')).files :
    ['VERSION', 'dashboard.html', 'collect.mjs', 'lib', 'tools', 'assets', 'release', 'alert', 'collectors', 'runtime', 'config.template.json'];
...
function resume(root, isolated) {
  if (!isolated && fs.existsSync(path.join(root, 'tools/install.mjs'))) {
    execFileSync(path.join(root, 'runtime/node.exe'), [path.join(root, 'tools/install.mjs'), '--skip-verify'], { windowsHide: true, timeout: 120000, stdio: 'pipe' });
  }
}
