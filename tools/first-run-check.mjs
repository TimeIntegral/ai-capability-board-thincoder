...
//
// 为什么单独一个文件：安装收尾（install.mjs）与首次配置向导（setup.mjs）都要做这一步，而且这一步
// 必须能被**单独调用**——install.mjs 会注册真实的计划任务与桌面快捷方式，不适合为了验证反复执行。
// 抽成模块后即可两处复用，也能自己跑：
//
//   node tools/first-run-check.mjs        跑一次采集并打印报告（退出码 0 = 都通，1 = 有平台没通）
...
// 重跑向导的提示语（多个地方要用，抽出来免得三处措辞不一致）
const RETRY_HINT = '双击「首次配置.bat」重新跑一遍';
...
export function summarize(state) {
  ...
  if (failedLabels.length) {
    ...
    verdict = {
      level: 'warn',
      text: `⚠ 还差一步：${needKeyLabels.join('、')} 还没配好 —— ${RETRY_HINT}，按提示把密钥补上就行。`,
    };
...
// 单独运行：node tools/first-run-check.mjs
