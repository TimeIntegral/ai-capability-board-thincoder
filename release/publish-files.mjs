...
export const 仅供开发 = [
  ...
  // 上传脚本单独列一条：它是唯一会**对外发东西**的开发工具（建 Release、传附件、发布），
  // 理由写得比别的构建脚本明白一点。实际覆盖来自上一条 `release/`（命中取首个匹配），
  // 这条是把「它必须留在开发层、永远不进用户包」写在明面上。
  ['release/upload-release.mjs', '发布上传：核对候选版 → 建 Release 草稿 → 传四个附件 → 发布 → 实测下载核对（只有维护者在本机跑）'],
